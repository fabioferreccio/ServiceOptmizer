...
namespace ServiceOptimizer.Classes
{
    class Windows81
    {
        ...
        public Windows81(ref byte op)
        {
            ...
            Console.WriteLine("7 - Convert CSV (ConfigCSV_w81) to JSON");
        ...
        public void convert_CsvToJson(ref List<BlackViperModel> listWin81, ref byte op)
        {
            try
            {
                string sourceFile = @"ConfigCSV_w81.csv";
                using (TextReader fileReader = File.OpenText(sourceFile))
                {
                    ...
                    csv.Configuration.RegisterClassMap(new StrBlackViper_Win81_ClassMap());
                    listWin81 = csv.GetRecords<BlackViperModel>().ToList<BlackViperModel>();
                    File.WriteAllText(@"ConfigJSON_w81.json", JsonConvert.SerializeObject(listWin81, new JsonSerializerSettings
                    {
